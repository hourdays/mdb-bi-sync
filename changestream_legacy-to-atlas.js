...
async function main() {
     ...
     */
    const source_uri = "mongodb://<user>:<password>@<ip>:<port>/?directConnection=true"; // Legacy 
    const dest_uri = "mongodb+srv://<user>:<password>@<atlas-cluster-url>/"; // Atlas
     ...
     */
    const source_client = new MongoClient(source_uri); // Legacy 
    const dest_client = new MongoClient(dest_uri); // Atlas
    ...
    try {
        ...
        /**
         * An aggregation pipeline that matches on new documents not inserted by Atlas change stream
         */
        const pipeline = [
            {
                '$match': {
                    'operationType': 'insert',
                    'fullDocument.sync_source': {$ne : 'atlas-changestream'}
                }
            }
        ];
...
async function monitorUsingEventEmitter(source_client, timeInMs = 60000, pipeline = [], dest_client) {
    ...
    changeStream.on('change', (next) => {
        ...
        fullDocument['sync_source'] = 'legacy-changestream';
        fullDocument['sync_dt'] = new Date();
        console.log(fullDocument);

        // Insert new document into destination cluster so here Atlas
